...
async function testSingleOcr() {
    ...
    if (!filePath) {
        console.log('用法: npx tsx scripts/test-ocr-single.ts <pdf-path> [page-count]')
        console.log('示例: npx tsx scripts/test-ocr-single.ts knowledge/raw/books/book.pdf 2')
        process.exit(1)
    }
    const pageCount = parseInt(process.argv[3] || '2', 10)
    ...
    try {
        ...
        execSync(`python scripts/ocr_worker.py "${filePath}" "${tempMdPath}" ${pageCount}`, { stdio: 'inherit' })
