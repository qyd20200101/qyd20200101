...
① 先问概念：用户用自己的话回答
② 纠错 + 补充：指出哪里对/错，给标准答案
③ 反面案例追问：问"如果没有它会怎样？""出了什么 bug？""怎么防御？"等场景题
```

**目的**：回答对字面不代表真理解，反面案例才能验证是否融会贯通。
...
手写题**不看代码完整性**，看三步：① 场景（什么时候用）→ ② 核心步骤（逻辑链路说清）→ ③ 代码（写得出最好，写不出但步骤说对也算掌握）。

用户回答时优先用场景提问而非直接给代码。用户写了代码但漏了细节，先指出"逻辑对了，xxx 处理对了"，再补齐遗漏。

### 新内容 vs 已有自评的区别处理

- **已有自评的 🔴🟡**：直接加固讲解 → 回写文件
- **推进学习计划的新内容**（自评里还没有的）：**必须先让用户用自己的话说一遍**（"你之前学过 X 吗？用一句话说说你的理解"），再根据盲区补充纠正。**禁止直接灌概念再检验**。
